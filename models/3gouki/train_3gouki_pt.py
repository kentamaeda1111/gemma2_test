...
# グローバル設定
DIALOGUE_JSON_PATH = "logs/dialogue/3gouki.json"  # 対話データのJSONファイルパス
...
training_args = TrainingArguments(
    output_dir="./model",
    num_train_epochs=30,
    learning_rate=1e-4,      # 8e-5から1e-4に増加
    weight_decay=0.06,
    warmup_ratio=0.15,       # 0.25から0.15に短縮
    ...
    report_to=[],
    run_name=None,
    per_device_train_batch_size=6,  # 4から6に増加
    per_device_eval_batch_size=3,   # 2から3に増加（train_batch_sizeの半分）
...
class TrainingMonitorCallback(TrainerCallback):
    ...
    def on_train_end(self, args, state, control, **kwargs):
        ...
        summary = {
            ...
            'final_loss': self.metrics_history['loss'][-1] if self.metrics_history['loss'] else "N/A",
            'best_combined_score': max(filter(None, self.metrics_history['combined_score'])) if self.metrics_history['combined_score'] else "N/A",
            'total_steps': len(self.metrics_history['step']),
            'final_epoch': self.metrics_history['epoch'][-1] if self.metrics_history['epoch'] else "N/A"
        }
        ...
        logging.info(f"Final loss: {summary['final_loss']}")
        logging.info(f"Best combined score: {summary['best_combined_score']}")
