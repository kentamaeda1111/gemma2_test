...
training_args = TrainingArguments(
    ...
    metric_for_best_model="eval_loss",  # perplexityではなくeval_lossを使用
    dropout=0.1,       # dropoutの追加
)
...
training_args = TrainingArguments(
    output_dir=MODEL_OUTPUT_DIR,  
    num_train_epochs=30,
    learning_rate=1e-4,  # 2e-4から1e-4に減少
    weight_decay=0.1,            # 0.06から0.1に減少
    warmup_ratio=0.15,  # 0.1から0.15に増加
    lr_scheduler_type="linear",  # cosineからlinearに変更
    evaluation_strategy="steps",
    eval_steps=50,  # 100から50に減少
    save_steps=50,  # 100から50に減少
    gradient_accumulation_steps=8,    # バッチサイズを小さくした分、これを8に増やして補完
    max_steps=-1,
    disable_tqdm=False,
    logging_dir=LOG_OUTPUT_DIR,   
