...
def preprocess_function(examples):
    ...
    for text, mask in zip(texts, examples['attention_mask']):
        ...
        # Create new attention mask (all values set to 1.0)
        new_mask = [1.0] * len(mask)
        
        # Special token masks are set to 1.0 (although they're already 1.0)
...
class TrainingMonitorCallback(TrainerCallback):
    ...
    def _save_training_metrics(self):
        ...
        # Moving average loss plot - 修正部分
        if self.metrics_history['moving_avg_loss']:
            # 移動平均のデータ点数に合わせてx軸のデータを調整
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        if logs:
            ...
            # 移動平均の計算と記録を修正
            if len(self.metrics_history['loss']) >= 10:  # 10個以上のデータポイントがある場合のみ計算
                window_size = 10
