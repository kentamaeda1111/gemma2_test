# https://claude.ai/chat/89f84366-8b0b-4b99-8928-09a601dfa879　を使ってkaggle仕様に修正
# あとファインチューンの差分をロードできるようにした。
# あとforinitialtestingのprompt挿入の機能もある
...
class ChatAI:
    ...
    def __init__(
        ...
        """
        Constructor optimized for Kaggle T4 environment
        Args:
        ...
        try:
            ...
            logger.info("Model loaded successfully with T4 optimizations")
    ...
    def _initialize_model(self, base_model, model_path, hf_token):
        """Initialize the model with T4 GPU optimized settings"""
        try:
            # Kaggle T4環境用の最適化設定
            load_config = {
                "device_map": "auto",  # AutoDeviceMappingが最適な分配を行います
                "torch_dtype": torch.float16,  # T4ではfloat16が最適
                "trust_remote_code": True,
                "token": hf_token,
                "max_memory": {
                    0: "12GB",  # 1台目のGPUに12GB割り当て
            ...
            self.model = PeftModel.from_pretrained(
                ...
                model_path,
                torch_dtype=load_config["torch_dtype"],
                device_map="auto"  # モデルを自動的に分割
            )

            logger.info(f"Model loaded successfully with optimized T4 settings")
    ...
    def generate_response(self, user_input: str, add_to_history: bool = True) -> str:
        ...
        try:
            ...
            with torch.cuda.amp.autocast():  # 自動混合精度を使用
